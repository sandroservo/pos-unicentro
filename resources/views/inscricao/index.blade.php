...
<body class="bg-gray-100">
        ...
        <form action="{{ route('inscricao.store') }}" method="POST" autocomplete="off">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="cpf" name="cpf" value="{{ old('cpf') }}" required maxlength="14"
                    class="w-full border rounded-lg px-3 py-2" placeholder="000.000.000-00">
                @error('cpf')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="date" id="data_nascimento" name="data_nascimento"
                        value="{{ old('data_nascimento') }}" required class="w-full border rounded-lg px-3 py-2">
                ...
                <div>
                    ...
                    <select id="ensino_medio" name="ensino_medio" required class="w-full border rounded-lg px-3 py-2">
                        ...
                        <option value="superior_completo"
                            {{ old('ensino_medio') == 'superior_completo' ? 'selected' : '' }}>Superior Completo
                        </option>
                        <option value="superior_incompleto"
                            {{ old('ensino_medio') == 'superior_incompleto' ? 'selected' : '' }}>Superior Incompleto
                        </option>
                        <option value="pos-graduado" {{ old('ensino_medio') == 'pos-graduado' ? 'selected' : '' }}>
                            Pós-Graduado</option>
                    </select>
                    @error('ensino_medio')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" id="cep" name="cep" value="{{ old('cep') }}" required
                        maxlength="9" class="w-full border rounded-lg px-3 py-2" placeholder="00000-000">
                    @error('cep')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    <label for="telefone_celular" class="block text-gray-700 font-bold mb-2">Telefone Celular
                        *</label>
                    <input type="tel" id="telefone_celular" name="telefone_celular"
                        value="{{ old('telefone_celular') }}" required maxlength="15"
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select id="tipo_aluno" name="tipo_aluno" required class="w-full border rounded-lg px-3 py-2">
                        <option value="">Selecione</option>
                        <option value="1" {{ old('tipo_aluno') == '1' ? 'selected' : '' }}>12x de R$ 225,00
                            (total R$ 2.700,00)</option>
                        <option value="2" {{ old('tipo_aluno') == '2' ? 'selected' : '' }}>15x de R$ 212,50
                            (total R$ 3.187,50</option>
                        <option value="3" {{ old('tipo_aluno') == '3' ? 'selected' : '' }}>18x de R$ 190,00
                            (total R$ 3.420,00)
                        </option>
                    </select>
                    @error('tipo_aluno')
                ...
                <div>
                    ...
                    <select id="pos_graduacao" name="pos_graduacao" required
                        class="w-full border rounded-lg px-3 py-2">
                        <option value="">Selecione</option>
                        @foreach ($cursos as $curso)
                            <option value="{{ $curso->nome }}"
                                {{ old('pos_graduacao') == $curso->nome ? 'selected' : '' }}>{{ $curso->nome }}
                            </option>
                        @endforeach
                    </select>
                    @error('pos_graduacao')
            ...
            <div class="mb-6">
                ...
                <input type="text" id="valor_mensalidade" name="valor_mensalidade"
                    value="{{ old('valor_mensalidade') }}" readonly
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div>
                    <label for="senha_confirmation" class="block text-gray-700 font-bold mb-2">Confirme a Senha
                        *</label>
                    <input type="password" id="senha_confirmation" name="senha_confirmation" required
                        autocomplete="new-password" class="w-full border rounded-lg px-3 py-2">
            ...
            <!-- Botão de envio -->
            <button type="submit" class="bg-blue-900 text-white w-full py-3 rounded-lg font-bold hover:bg-blue-700">
                Enviar Dados
            </button>
    ...
    {{-- <script>
        function preencherCampos() {
            document.getElementById('nome').value = 'Sandro de Souza Silva';
            document.getElementById('cpf').value = '01661255396';
            ...
            document.getElementById('telefone_celular').value = '(99)9';
